...
module pen_config_controller #(
    ...
    // 버튼 이벤트 (button_debounce의 1클럭 펄스)
    input  logic       i_btn_eraser,
    input  logic       i_btn_mode,
    input  logic       i_btn_size,

    // 현재 펜 상태 (uart_packet_sender와 드로잉 파이프라인이 공유)
    ...
    output logic       o_freeze,        // 캡처 모드 (GUI 전용, 버튼 없음)
    output logic       o_clear          // CLEAR_STRETCH 클럭 폭으로 늘린 clear
);
