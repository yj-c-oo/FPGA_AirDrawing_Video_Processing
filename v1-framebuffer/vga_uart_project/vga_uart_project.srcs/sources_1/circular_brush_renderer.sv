...
// Rasterizes one brush stamp and generates clipped Canvas write addresses.
// texture_shape map:
// 0: spray small, 1: spray medium, 2: spray large, 3: diagonal thin, 4: diagonal wide
module circular_brush_renderer (
    ...
    always_comb begin
        start_texture_on          = texture_enable && color[3];
        ...
        if (start_texture_on) begin
            case (texture_shape)
                3'd0: begin
                    start_texture_kind  = TEXTURE_SPRAY;
                    start_radius        = texture_radius(radius, 5'd2, 5'd5);
                    start_spray_density = 3'd1;
                end
                3'd1: begin
                    start_texture_kind  = TEXTURE_SPRAY;
                    start_radius        = texture_radius(radius, 5'd4, 5'd7);
                    start_spray_density = 3'd2;
                end
                3'd2: begin
                    start_texture_kind  = TEXTURE_SPRAY;
                    start_radius        = texture_radius(radius, 5'd6, 5'd9);
